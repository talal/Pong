...
public class JwtAuthController {
	...
	public JwtAuthController(AuthenticationManager am, JwtTokenService jts, JwtBlacklistService jbs,
							 UserDetailsService uds, UserRepository repo, PasswordEncoder enc) {
	...
	public ResponseEntity<?> signup(@RequestBody SignupRequest req) {
		if (req.email() == null || req.password() == null ||
			...
			return ResponseEntity.badRequest().body(Map.of("error", 
					"E-Mail address and password required"));
		}
		if (userRepo.existsByEmail(req.email())) {
			return ResponseEntity.status(409).body(Map.of("error", 
					"E-Mail address already assigned to another user"));
		}
		User u = new User();
